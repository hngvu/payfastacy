...
const nanoid = customAlphabet(
  ...
  parseInt(process.env.CONTENT_LENGTH) || 11
);
...
async function generateUniqueContent(db) {
  ...
  if (!isUnique) {
    throw new Error(
      "Failed to generate unique content after multiple attempts"
    );
  }
...
export async function routes(fastify, options) {
      ...
      try {
        ...
        if (!amount || !ref) {
          return reply.code(400).send({
            success: false,
            error: "Amount and ref are required",
          });
        }

        // Check if ref already exists
        const existingRef = await request.server.db
          .select()
          .from(payment)
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        const { content: webhookContent, transferAmount, id } = request.body;
        ...
        const payments = await request.server.db
          .select()
          .from(payment)
          .where(
            and(eq(payment.amount, transferAmount), eq(payment.status, false))
          )
          .then((results) =>
            results.filter((p) => webhookContent.includes(p.content))
          );
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        if (from) {
          conditions.push(
            gte(payment.createdAt, new Date(from + "T00:00:00+07:00"))
          );
        }

        if (to) {
          conditions.push(
            lte(payment.createdAt, new Date(to + "T23:59:59+07:00"))
          );
        }
        ...
        return reply.send({
          success: true,
          data: payments.map(p => {
                const { status: booleanStatus, ...rest } = p; // Remove the boolean status
                return {
                    status: booleanStatus ? 'paid' : 'unpaid', // Use string status
                    ...rest
                };
            }),
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        const sepayTxnId = await request.server.db
          .select()
          .from(payment)
          .where(eq(payment.txnId, parseInt(txnId)))
          .limit(1)
          .then((results) => (results.length > 0 ? results[0].txnId : null));
        ...
        const response = await fetch(
          `https://my.sepay.vn/userapi/transactions/details/${sepayTxnId}`,
          {
            method: "GET",
            headers: {
